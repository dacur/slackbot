...
class CodeReview {
  ...
  bootstrapFromStorage(storageData) {
    for (let index in JSON.parse(storageData)) {
      this[index] = storageData[index];
      console.log(index);
    }
    this._syncRedis();
    return this;
  ...
  _syncRedis() {
    ...
    redisAdapter.set(this.pullRequestUrl, JSON.stringify(data));
  }
