...
class CodeReview {
  ...
  _checkCommented() {
    ...
    if ( reviewers.length > this.numberOfReviwers ) {
      ...
      reviewers.each((reviewer, index) => {
        if (index > this.numberOfReviwers-1) Slack.addReactionToCRMessage(this, this.EMOJI_REVIEWED);
      })
    }
  }
    ...
    this.poll = setTimeout(() => {
      ...
      Github.getPRChanges(prParamsEvents, prParamsComments)
        .then((responses) => {
          ...
          throw new Error(CodeReview + err);
        });
    }, 500);
  }
}
